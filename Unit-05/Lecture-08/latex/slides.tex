...
\begin{frame}[shrink=7]{The family of regression models}
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    \bottomrule
  \end{tabular}\\[0.2em]
  \scriptsize\emph{Closed form}: the answer is a formula you evaluate once.
  \emph{Iterative}: repeat an update until it stops changing.
  \emph{Convex}: bowl-shaped --- \hyperlink{words}{defined properly in
  Section~4}.
  \end{center}
  \vspace{0.2em}
...
\begin{frame}[label=words,shrink=6]{Four words the rest of the lecture leans on}
  \begin{columns}[T]
    \begin{column}{0.55\textwidth}
      \small
      \begin{itemize}
        \item \textbf{Closed form}: the answer is a formula --- plug the data
          in, compute once, done. \emph{Iterative} methods (gradient descent)
          repeat a step until it stops changing.
        \item \textbf{Stationary point}: every partial derivative is zero, so
          the surface is flat there. It may be a minimum, a maximum or a
          saddle.
        \item \textbf{Local \emph{vs} global minimum}: lowest in its own
          neighbourhood, \emph{vs} lowest anywhere.
        \item \textbf{Convex}: the straight chord joining any two points of
          the graph never dips \emph{below} the graph --- a bowl.
          \textbf{Strictly convex}: the chord is strictly above, so the bowl
          has exactly one bottom.
      \end{itemize}
    \end{column}
    \begin{column}{0.42\textwidth}
      ...
      \begin{tikzpicture}[scale=0.85]
        ...
        \draw[very thick,UPESBlueDark,domain=-1.9:1.9,smooth]
          plot (\x,{0.45*\x*\x+0.2});
        \draw[UPESOrange,thick] (-1.5,{0.45*2.25+0.2}) -- (1.2,{0.45*1.44+0.2});
        \fill[UPESOrange] (-1.5,{0.45*2.25+0.2}) circle (2pt)
          (1.2,{0.45*1.44+0.2}) circle (2pt);
        \node[font=\scriptsize,align=center] at (0,2.2)
          {convex: chord always above};
        \fill[green!50!black] (0,0.2) circle (2.5pt);
        \node[font=\tiny,below] at (0,0.15) {one minimum};
        % not convex
        \begin{scope}[yshift=-3.3cm]
          \draw[->,gray] (-2.1,0) -- (2.2,0);
          \draw[very thick,UPESBlueDark,domain=-1.95:1.95,smooth,samples=60]
            plot (\x,{0.28*\x*\x*\x*\x-0.9*\x*\x+0.25*\x+1.0});
          \draw[UPESOrange,thick] (-1.35,{0.28*1.35^4-0.9*1.35^2-0.25*1.35+1.0})
            -- (1.25,{0.28*1.25^4-0.9*1.25^2+0.25*1.25+1.0});
          \node[font=\scriptsize,align=center] at (0,2.2)
            {not convex: chord dips below};
          \node[font=\tiny] at (-1.35,-0.35) {global min};
          \node[font=\tiny] at (1.3,-0.35) {local min};
        \end{scope}
      \end{tikzpicture}
    \end{column}
  \end{columns}
  \begin{alertblock}{Why convexity matters today}
    \small For a convex function \textbf{every stationary point is a global
    minimum}. So ``set the derivatives to zero'' finds the best line, not
    merely a locally good one.
  \end{alertblock}
\end{frame}
    ...
    \begin{column}{0.48\textwidth}
      \begin{exampleblock}{Why squares win}
        ...
        \begin{enumerate}
          ...
          \item Under Normal (Gaussian) noise it is also the
            \emph{maximum-likelihood} answer --- the parameters that make the
            observed data most probable. Lecture 9.
        \end{enumerate}
      \end{exampleblock}
    \end{column}
  \end{columns}
...
\begin{frame}[shrink=6]{Tools for Step 4: the Hessian, positive definite}
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{block}{The Hessian $H$}
        \small The matrix of all second partial derivatives: entry $(j,k)$ is
        $\partial^2 f/\partial b_j\,\partial b_k$. It records how the surface
        \textbf{curves} in every direction --- the many-variable version of
        $f''(x)$.
      \end{block}
      \begin{block}{The test you already know}
        \small One variable: $f'(x)=0$ and $f''(x)>0$ $\Rightarrow$ minimum.\\
        Several variables: $\nabla f = 0$ and $H$ \textbf{positive definite}
        $\Rightarrow$ minimum.
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{Positive definite}
        \small A symmetric $H$ is positive definite if
        $v^{\!\top}Hv > 0$ for every non-zero direction $v$: walk any way and
        the surface curves \emph{up}. Two equivalent checks:
        \begin{itemize}
          \item $2\times2$: $H_{11} > 0$ and $\det H > 0$;
          \item all \textbf{eigenvalues} of $H$ are positive (they are the
            curvatures along the bowl's principal axes).
        \end{itemize}
      \end{block}
      \begin{block}{A saddle, for contrast}
        \small $f(u,v) = u^2 - v^2$ is flat at the origin, but curves up
        along $u$ and down along $v$: $H = \mathrm{diag}(2,-2)$, not
        positive definite. A horse's saddle.
      \end{block}
    \end{column}
  \end{columns}
  \begin{center}\small
    $H$ positive definite everywhere $\Rightarrow$ the function is
    \textbf{strictly convex} $\Rightarrow$ its one stationary point is the
...
\begin{alertblock}{Not ``close''. Identical, to machine precision}
  ...
  because it goes through an SVD (singular value decomposition, a
  factorisation of $X$ that never forms $X^{\!\top}\!X$). \textbf{The library is not doing anything
    ...
    \begin{column}{0.49\textwidth}
      \begin{block}{From the first normal equation}
        ...
        \[ b_0 = \bar y - b_1\bar x \;\Rightarrow\;
           \hat y(\bar x) = \bar y. \]
        The line passes through the centroid $(\bar x, \bar y)$, the point of
        means.
      \end{block}
    \end{column}
    \begin{column}{0.49\textwidth}
      \begin{block}{From the second}
        ...
        The residual vector is orthogonal (dot product zero) to the predictor
        \emph{and} to the fitted values --- there is no linear signal left in $e$.
      \end{block}
...
\begin{frame}[shrink=6]{How good is the slope? $S_{xx}$ is the answer}
  Collect a fresh sample, refit, and $b_1$ comes out a little different.
  $\mathrm{Var}(b_1)$ measures how much; its square root is the
  \textbf{standard error} of $b_1$. Under the four assumptions one can show
  \[
...
\begin{frame}[shrink=8]{Drawn: the same result, twice}
  ...
  \end{center}
  \begin{center}\small
    Left: four \emph{sampling distributions} of $b_1$ (histograms of the
    4000 refitted slopes), all centred on the true value $2.0$ --- least
    squares is \emph{unbiased} (right on average) whatever the spread --- but
    with very different widths. Right: the measured widths against
...
\begin{frame}[shrink=5]{The same derivation, written once for any number of columns}
  ...
  \begin{exampleblock}{}
    ...
    $2X^{\!\top}\!X$, positive definite exactly when $X$ has \textbf{full
    column rank}: no column is a combination of the others (here: $x$ is not
    constant).
  \end{exampleblock}
  \begin{center}\small
...
\begin{center}\small
  ...
  better numerics when $X^{\!\top}\!X$ is nearly singular (almost not
  invertible, determinant close to zero).
\end{center}
\end{frame}
...
\begin{alertblock}{The middle point cannot change the slope. At all}
  \footnotesize \emph{Leverage}: how far a row's own $y$ can pull the fitted
  slope, set by where it sits in $x$. The middle point's leverage is
  \textbf{exactly zero}, because
  ...
  \textbf{two points out of five hold $80\%$ of the influence}.
\end{alertblock}
\end{frame}

...
\begin{tabular}{@{}lp{0.72\textwidth}@{}}
  ...
  \midrule
    SLR & simple linear regression: one predictor, a straight line\\
    SSE & sum of squared errors, $\sum_i (y_i-\hat y_i)^2 = \sum_i e_i^2$\\
    SST & total sum of squares, $\sum_i (y_i-\bar y)^2$\\
    SSR & regression (explained) sum of squares, $\sum_i(\hat y_i-\bar y)^2$\\
    SAE & sum of absolute errors, $\sum_i |e_i|$\\
    LAD & least absolute deviations: the line that minimises SAE\\
    RMSE & root mean squared error, $\sqrt{\mathrm{SSE}/n}$, in the units of $y$\\
    GD, SGD & gradient descent; stochastic gradient descent\\
    SVD & singular value decomposition (a matrix factorisation)\\
    sd & standard deviation\\
    BMI & body mass index\\
    $k$-NN & $k$-nearest neighbours\\
    L3, L9, \dots & Lecture 3, Lecture 9, \dots\ of this course\\
  OLS & ordinary least squares --- the method of this lecture\\
  ANOVA & analysis of variance (the SST $=$ SSR $+$ SSE split, Lecture 9)\\
  CO1, CO2, \dots & course outcomes: the official outcomes of this course that
    each learning outcome below serves\\
  \midrule
    $\bar x,\ \bar y$ & means of $x$ and $y$ (\texttt{xbar}, \texttt{ybar} in printed output)\\
    $\hat y_i$ & fitted value $b_0 + b_1x_i$ (\texttt{yhat})\\
    $e_i$ & residual $y_i - \hat y_i$\\
    $S_{xx},\ S_{xy},\ S_{yy}$ & $\sum(x_i-\bar x)^2$, $\sum(x_i-\bar x)(y_i-\bar y)$, $\sum(y_i-\bar y)^2$ (\texttt{Sxx}, \dots)\\
    $X^{\!\top}$ & transpose of $X$ (\texttt{X'} or \texttt{X.T} in code)\\
  \bottomrule
\end{tabular}
\end{center}
...
By the end of this lecture you should be able to (the tag in brackets is the
course outcome, CO, that each item serves):
...
The last column uses three words that recur all lecture. A \textbf{closed
form} answer is a formula: substitute the data, compute once, and you are
done. An \textbf{iterative} method (gradient descent, Lecture 3) instead
repeats an update until the numbers stop changing. \textbf{Convex} means
bowl-shaped; it is defined properly in Section~\ref{sec:words}.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Choice of fitting method.} Least squares, maximum likelihood,
    ...
    first; Lecture 9 shows the first two coincide under Normal (Gaussian)
    noise. (\emph{Maximum likelihood} picks the parameter values under which
    the observed data would have been most probable.)
...
cross-validated RMSE (root mean squared error, $\sqrt{\mathrm{SSE}/n}$,
in the units of $y$) of $62.6$ against $77.0$ for the null model that always
...
\subsection{Four words you need before the derivation}
\label{sec:words}

The rest of the lecture leans on four pieces of vocabulary from calculus and
optimisation. They are defined here once.

\begin{description}[leftmargin=0pt]
  \item[Closed form.] The answer can be written as a formula in the data ---
    plug in, compute once. The opposite is an \emph{iterative} method, which
    improves a guess step by step until it stops changing.
  \item[Stationary point.] A point where every partial derivative of the
    function is zero, so the surface is flat there. A stationary point can be
    a minimum, a maximum, or a \emph{saddle} (a minimum in one direction and
    a maximum in another; see Section~\ref{sec:soc}).
  \item[Local and global minimum.] A local minimum is lower than every
    point \emph{near} it; a global minimum is lower than every point
    \emph{anywhere}. A function can have many local minima and only one
    global minimum value.
  \item[Convex function.] $f$ is convex if the straight chord joining any
    two points of its graph never lies \emph{below} the graph:
    \[
      f\bigl(\lambda u + (1-\lambda)v\bigr) \;\le\; \lambda f(u) +
        (1-\lambda) f(v)\qquad\text{for all } u, v \text{ and }
        0\le\lambda\le1 .
    \]
    Picture a bowl. $f$ is \emph{strictly} convex if the inequality is strict
    whenever $u \ne v$: the bowl then has exactly one bottom. In one
    variable, $f''(x) > 0$ everywhere guarantees strict convexity (for
    example $x^2$); $x^4 - x^2$ is not convex --- it has two separate
    dips.
\end{description}
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=1.0]
    \begin{scope}
      ...
      \draw[very thick,blue!60!black,domain=-1.9:1.9,smooth]
        plot (\x,{0.45*\x*\x+0.2});
      \draw[orange,thick] (-1.5,{0.45*2.25+0.2}) -- (1.2,{0.45*1.44+0.2});
      \fill[orange] (-1.5,{0.45*2.25+0.2}) circle (2pt)
        (1.2,{0.45*1.44+0.2}) circle (2pt);
      \fill[green!50!black] (0,0.2) circle (2.5pt);
      \node[font=\small] at (0,2.3) {convex: every chord lies above};
      \node[font=\footnotesize,below] at (0,0.1) {one minimum};
    \end{scope}
    \begin{scope}[xshift=5.5cm]
      \draw[->,gray] (-2.1,0) -- (2.2,0);
      \draw[very thick,blue!60!black,domain=-1.95:1.95,smooth,samples=60]
        plot (\x,{0.28*\x*\x*\x*\x-0.9*\x*\x+0.25*\x+1.0});
      \draw[orange,thick] (-1.35,{0.28*1.35^4-0.9*1.35^2-0.25*1.35+1.0})
        -- (1.25,{0.28*1.25^4-0.9*1.25^2+0.25*1.25+1.0});
      \node[font=\small] at (0,2.3) {not convex: this chord dips below};
      \node[font=\footnotesize] at (-1.35,-0.35) {global min};
      \node[font=\footnotesize] at (1.3,-0.35) {local min};
    \end{scope}
  \end{tikzpicture}
  \caption{Left: a convex function. Every chord (orange) lies above the
    graph, and there is one minimum. Right: a function that is not convex.
    The chord between its two dips passes below the graph, and ``the
    derivative is zero'' holds at a local minimum, the global minimum and the
    hump between them.}
  \label{fig:convex}
\end{figure}

\begin{keyidea}[Why convexity matters]
For a convex function \textbf{every stationary point is a global minimum}.
So if the least-squares criterion is convex --- and
Section~\ref{sec:soc} proves it is --- then ``set the derivatives to zero''
finds the best line, not merely a locally good one.
\end{keyidea}
...
\begin{itemize}[leftmargin=*]
  ...
  \item $\sum_i |e_i|$, the sum of absolute errors (SAE); the line that
    minimises it is the least absolute deviations (LAD) line. Sensible and robust to
...
A vanishing gradient marks a minimum, a maximum or a saddle. In one
variable you would settle the question with the second-derivative test:
$f'(x)=0$ and $f''(x)>0$ means a minimum. With several variables the second
derivative becomes a matrix.

\begin{description}[leftmargin=0pt]
  \item[Hessian.] The matrix $H$ of all second partial derivatives, with
    entry $(j,k)$ equal to $\partial^2 f/\partial b_j\,\partial b_k$. It is
    symmetric, and it records how the surface curves in every direction.
  \item[Positive definite.] A symmetric matrix $H$ is positive definite if
    $v^{\!\top}Hv > 0$ for every non-zero vector $v$. Geometrically: whichever
    direction $v$ you walk from the stationary point, the surface curves
    \emph{upward}. For a $2\times2$ matrix it is enough to check that
    $H_{11} > 0$ and $\det H > 0$. Equivalently, all the \emph{eigenvalues}
    of $H$ are positive; the eigenvalues are the curvatures along the
    principal axes of the bowl.
  \item[Saddle.] A stationary point where the surface curves up in some
    directions and down in others. The standard example is
    $f(u,v) = u^2 - v^2$ at the origin: $H = \mathrm{diag}(2,-2)$ has one
    positive and one negative eigenvalue, so it is not positive definite.
\end{description}

The multivariable test is then: gradient zero and Hessian positive definite
$\Rightarrow$ strict local minimum. If the Hessian is positive definite
\emph{everywhere}, the function is strictly convex and that minimum is the
unique global one (Section~\ref{sec:words}).

Differentiate \eqref{eq:d0} and \eqref{eq:d1} again:
\[
...
has \textbf{full column rank}, meaning no column of $X$ is a linear
combination of the others --- the general form of the ``not all $x_i$
equal'' condition (a constant $x$ column would be a multiple of the column
of ones). Note that \eqref{eq:matne} never mentions the number of columns:
...
$X^{\!\top}\!X$ is close to singular (almost non-invertible: its
determinant is near zero, so the inverse is huge and unreliable) --- which, as
Lecture 6 showed with the
...
(SVD) --- a factorisation of $X$ itself that solves least squares without
ever forming $X^{\!\top}\!X$, which is numerically safer.
...
$(\bar x, \bar y)$, the \textbf{centroid} (the point of means) of the data. This is a good sanity check on any hand computation: if
...
\begin{equation}
  ...
  \label{eq:orthyhat}
\end{equation}
Two vectors are \textbf{orthogonal} when their dot product is zero, which is
exactly what \eqref{eq:orthx} and \eqref{eq:orthyhat} say. Geometrically the
residual vector is orthogonal both to the column of ones and
...
they are random quantities: collect a fresh sample, refit, and you get
slightly different numbers. The distribution of $b_1$ over all such repeated
samples is its \textbf{sampling distribution}; its standard deviation,
$\sqrt{\mathrm{Var}(b_1)}$, is the \textbf{standard error} of $b_1$. Under the four assumptions of
...
\begin{equation}
  ...
  \label{eq:varb}
\end{equation}
(The first says least squares is \emph{unbiased}: on average over
repeated samples it hits the true value; the Gauss--Markov theorem
...
One hundred steps of full-batch gradient descent (GD in the printout) reproduce
...
\begin{itemize}[leftmargin=*]
  ...
  \item \emph{Seeing Theory}, Chapter 6: Regression Analysis (Brown
    ...
    datasets} and watch the OLS (ordinary least squares) table update live, and continues into
    correlation and ANOVA (analysis of variance).

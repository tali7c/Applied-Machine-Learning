...
with holes in them to a result you could defend in the project quiz.
...
\paragraph{Project.} 15 marks in two pieces. The \textbf{repository
submission} is 5 marks awarded to the group --- identical for every member ---
for the completeness, correctness, reproducibility, documentation \emph{and
commit history} of the GitHub repository, which is the only thing submitted.
The \textbf{project quiz} is 10 marks and is \emph{marked individually} --- a
written paper on your own group's project, authored in one set per theme, with
every member sitting it separately and no oral component. The remaining
milestones are the final submission, which is the repository URL, and then the

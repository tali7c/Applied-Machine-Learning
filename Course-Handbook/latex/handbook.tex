...
Project --- individual & 10 & A written quiz on your own group's project; \textbf{marked individually}\\
Project --- group & 5 & The state and commit history of your group's GitHub repository; the same mark for every member\\
...
Eight project themes are offered, P01--P08. You form your own group of
\textbf{4 to 8 students} and choose one; \textbf{at most two groups} may take the
same theme, first come, first served. The full briefs are in
\texttt{projects/} in the course repository --- one folder per theme, plus the
project handbook,
which you should read before choosing.
...
\midrule
Project quiz & 10 & A \textbf{written paper} on your group's project, one set per theme. \textbf{Marked individually} --- every member sits it separately.\\
Repository & 5 & State \emph{and commit history} of the group's GitHub repository: process, completeness, correctness, reproducibility, documentation. \textbf{Identical mark for every member.}\\
\bottomrule
\end{tabular}
\end{center}
...
2 & Groups formed, theme selected, \textbf{GitHub repository created}\\
3 & Checkpoint 1 --- data secured, baseline model (tag \texttt{checkpoint-1})\\
4 & Checkpoint 2 --- full pipeline, initial results (tag \texttt{checkpoint-2})\\
5 & Final submission --- \textbf{the repository URL, and nothing else}\\
6 & Individual written project quiz\\
\bottomrule
\end{tabular}
\end{center}
...
happens through the individual written quiz.

You submit only a \textbf{GitHub repository URL}, and its \textbf{commit history
is graded}: created in Unit IV, at least 20 commits across at least six distinct
weeks, no more than 40\,\% of them in the final week, every member committing
from their own account, and both checkpoint tags pushed on time. A repository
that appears in one to three commits in the last few days scores at most 1.5 of
the 5 group marks, whatever is inside it.
\end{keyidea}

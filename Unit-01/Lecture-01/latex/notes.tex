...
Almost everything in these notes is computed from \textbf{one dataset}: the
\texttt{load\_iris} that ships inside scikit-learn, 150 flowers with four
measurements each. Nothing is downloaded, so all of it runs on the campus
network. Where a demonstration needs data iris cannot supply --- a curve to
over-fit, two features on absurdly different scales, five thousand columns of
pure noise --- the notes generate it, and say so.

Wherever a listing below depends on chance, \textbf{the seed is 0}. Every split
is written \texttt{random\_state=0} and every generator
\texttt{np.random.default\_rng(0)}, shown in the listing rather than hidden in
a helper, so you can see it and copy it. (The two synthetic \emph{figures} ---
the over-fitting curve and the scaling demonstration --- keep their own fixed
seeds, because there the seed defines the data rather than the experiment.)
...
1.13 for degree 3 --- it is the worst model available, and training error ranked
it best.
...
the standard deviation (0.134) is the signal that something is wrong.

...
Everything in these notes is computed from \textbf{one dataset} and
\textbf{one seed}. The dataset is the \texttt{load\_diabetes} above, with the
ten features and the target each standardised to zero mean and unit variance.
The seed is \textbf{0}: every random number generator below is created as
\texttt{np.random.default\_rng(0)}, and every listing that shuffles or samples
shows that line rather than hiding it.
...
\begin{lstlisting}
def run_decay(eta0=0.02, epochs=30, decay=True):
...
\end{lstlisting}
\begin{codeout}
SGD (B=1, 30 epochs), constant step 0.02   MSE 0.5319
SGD (B=1, 30 epochs), decayed step         MSE 0.4835
least-squares optimum                      MSE 0.4823
\end{codeout}

With a constant step the run stalls at $0.5319$ and stays there --- it is not
converging slowly, it is orbiting. Decaying the step closes $97\%$ of the
remaining gap, reaching $0.4835$ against an optimum of $0.4823$.
...
mini   B= 32             14          0.7377            0.4860
SGD    B=  1            442          0.5355            0.4969
...
still a long way from $0.4823$, while the mini-batch run has essentially
finished at $0.4860$ and SGD is parked on its noise floor at $0.4969$. And
after a \emph{single} epoch SGD is already at $0.5355$ --- better than batch
gradient descent manages in thirty.
...
\end{lstlisting}
\begin{codeout}
SGD reaches MSE 0.5355 after a single epoch.
Full-batch GD needs 71 epochs to reach the same MSE.
\end{codeout}

\textbf{Seventy-one passes over the data against one.} Both spend the same
...
descending and becomes a band of jitter averaging $0.50$.
...
\end{lstlisting}
\begin{codeout}
B=   1   442 updates/epoch     1.198 ms per epoch
B=  32    14 updates/epoch     0.058 ms per epoch
B= 442     1 updates/epoch     0.005 ms per epoch
\end{codeout}

Identical arithmetic; roughly \textbf{an order of magnitude} more wall-clock
cost for $B=1$ than for $B=32$ --- about $21\times$ on the machine that
produced the block above. And $B=32$ reached MSE $0.4860$ after thirty epochs
against SGD's $0.4969$ --- a slightly \emph{better} answer as well as a much
cheaper one, because at the same learning rate the larger batch also settles on
a lower noise floor.

\begin{caution}[Timings are the one thing a seed cannot fix]
Every other number in these notes is reproducible: fixed dataset, fixed seed,
same answer on your machine as on mine. The three millisecond figures above are
not. They depend on your processor, your BLAS build and what else is running,
and repeated runs on the same machine varied between roughly $20\times$ and
$30\times$ for the same comparison.

So quote the \emph{ratio} and never the timing --- and note that the update
counts beside them, $442$ against $14$ per epoch, are exact: $31.6\times$
fewer, on any machine. When you need a defensible speed claim, count operations
rather than seconds.

On a GPU the effect is far larger: a device with thousands of cores is almost
entirely idle processing one example at a time, and batch sizes of $256$ or
$1024$ can cost no more wall-clock time per update than a batch of $32$.
\end{caution}
...
\begin{lstlisting}
def run_mom(B, beta, eta, epochs=5):
    rng = np.random.default_rng(0)             # the one seed for this lecture
    w, v = np.zeros(p), np.zeros(p)
    for _ in range(epochs):
        order = rng.permutation(n) if B < n else np.arange(n)
...
mini-batch B=32, eta=0.005, 5 epochs
   beta=0.00   MSE 0.5356
   beta=0.90   MSE 0.4885
   beta=0.99   MSE 0.5321
least-squares optimum   MSE 0.4823
\end{codeout}

Read the three rows carefully. Going from $\beta = 0$ to $\beta = 0.9$ moves
the five-epoch result from $0.5356$ to $0.4885$ --- seven eighths of the
remaining gap to the optimum, closed by one line. Going on to $\beta = 0.99$
throws that gain straight back: at $0.5321$ it finishes level with no momentum
at all, because an effective learning rate of $100\eta$ makes the run overshoot
and spend the five epochs recovering.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Per epoch, small batches win overwhelmingly. SGD reached MSE $0.5355$
    in one epoch; full-batch gradient descent needed \textbf{$71$ epochs} for
  ...
  \item Mini-batch wins in practice because it vectorises. $B=32$ reached a
    slightly better MSE than $B=1$ ($0.4860$ against $0.4969$) and ran $31$
    times faster per epoch on the same machine.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item SGD with a constant step size stalled at MSE $0.5319$ while the optimum
...
from $0.5319$ to $0.4835$; (ii) \textbf{increase the batch size}, which divides
the noise by $\sqrt{B}$ --- at $\eta = 0.005$, thirty epochs of $B=32$ reached
$0.4860$ where $B=1$ stalled at $0.4969$. Both
...
went from $0.4885$ at $\beta = 0.9$ to $0.5321$ at $\beta = 0.99$ --- level
with the $0.5356$ that no momentum at all produced. To hold the effective

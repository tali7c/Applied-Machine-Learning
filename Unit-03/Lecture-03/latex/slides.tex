  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      \begin{block}{The data}
        \small scikit-learn's bundled \texttt{load\_diabetes}: 442 patients,
        10 features, features and target both standardised. Nothing is
        downloaded.
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      \begin{block}{The randomness}
        ...
        \texttt{np.random.default\_rng(0)}. Every listing that shuffles or
        samples shows it.
      \end{block}
    \end{column}
  \end{columns}
...
\begin{alertblock}{$\sum \eta_t = \infty$ and $\sum \eta_t^2 < \infty$}
  ...
  damp the noise. $\eta_t = \eta_0/(1+ct)$ does both --- and closed $97\%$ of
  the remaining gap here.
\end{alertblock}
\end{frame}
...
MSE \textbf{0.5355} after \emph{one} epoch.
...
\onslide<3->
\begin{center}\small
  \textbf{Seventy-one passes against one} (optimum $0.4823$). All of it comes
  from updating 442 times per pass instead of once.
\end{center}
\end{frame}
...
$B=32$ does $32\times$ the arithmetic per update that $B=1$ does, and after 30
epochs it is at MSE 0.4860 against $B=1$'s 0.4969.
...
\end{outblk}
\onslide<3->
\begin{alertblock}{$B=32$ is an order of magnitude faster than $B=1$}
  \footnotesize Same arithmetic, a slightly better MSE: one vectorised matrix
  operation instead of 32 tiny ones. The update counts are exact --- $442$
  against $14$, so $31.6\times$ fewer. The milliseconds are the one thing here
  a seed cannot pin down. Quote the ratio, never the timing.
\end{alertblock}
\end{frame}
      ...
      \begin{exampleblock}{Better than both ends, not a compromise}
        ...
        \begin{itemize}
          \item Keeps almost all of SGD's update frequency --- 14 per epoch was
            enough here.
          \item Recovers vectorised arithmetic: $31.6\times$ fewer updates per
            epoch, and roughly an order of magnitude in wall-clock time on a
            laptop --- far more on a GPU.
...
\onslide<2->
\begin{center}\small
  $\beta = 0.99$ throws the whole gain away: the effective step is $100\eta$,
  the run overshoots, and it finishes \emph{level with no momentum at all}.
\end{center}
\end{frame}
...
\begin{frame}[shrink=12]{Summary}
  \begin{enumerate}\small
    ...
    \item Per epoch, small batches win: SGD reached $0.5355$ in one epoch;
      batch GD needed \textbf{71}.
    \item Noise falls as $1/\sqrt{B}$, cost grows as $B$. $B{=}32$ measured
      $5.9\times$ less noise than $B{=}1$ for $32\times$ the arithmetic.
    \item \textbf{Mini-batch wins because it vectorises}: $0.4860$ against
      SGD's $0.4969$, with $31.6\times$ fewer updates per epoch.

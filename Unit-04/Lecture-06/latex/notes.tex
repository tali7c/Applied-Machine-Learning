...
\subsection{Two measures used throughout: correlation and $R^2$}
\label{sec:r2corr}

Almost every experiment in this lecture is read through one of two numbers.
Both are defined here once, so that the rest of the notes can simply quote
them.

\paragraph{Correlation.} The (Pearson) correlation of two columns $x$ and $y$
over $n$ rows is
\begin{equation}
  ...
  \label{eq:corr}
\end{equation}
It always lies in $[-1, 1]$. It is $+1$ when the points lie exactly on a
rising straight line, $-1$ on a falling one, and near $0$ when there is no
\emph{straight-line} tendency. It does not change if you rescale or shift
either column, which is why it is written without units. In NumPy it is
\texttt{np.corrcoef(x, y)[0, 1]}; in pandas, \texttt{df.corr()}.

\paragraph{The coefficient of determination, $R^2$.} For a model that
predicts $\hat y_i$,
\begin{equation}
  R^2 = 1 - \frac{\mathrm{SS}_{\mathrm{res}}}{\mathrm{SS}_{\mathrm{tot}}}, \qquad
  \mathrm{SS}_{\mathrm{res}} = \sum_{i=1}^{n} (y_i - \hat y_i)^2, \quad
  \mathrm{SS}_{\mathrm{tot}} = \sum_{i=1}^{n} (y_i - \bar y)^2 .
  \label{eq:r2}
\end{equation}
$\mathrm{SS}_{\mathrm{tot}}$ is the error of the laziest possible model, one
that always predicts the mean. So $R^2 = 1$ is a perfect fit, $R^2 = 0$ is no
better than the mean, and $R^2 < 0$ --- which Lecture~5 met with listwise
deletion --- is \emph{worse} than the mean. On held-out data it can be as
negative as the model is bad; there is no lower limit. Every scikit-learn
regressor's \texttt{.score(X, y)} returns it, as do
\texttt{sklearn.metrics.r2\_score(y, y\_pred)} and
\texttt{cross\_val\_score(..., scoring="r2")}.

\paragraph{How the two are related.} For a straight line fitted by least
squares on a \emph{single} input, $R^2 = r^2$. With several inputs there is no
single $r$, and $R^2$ is the number to use.

\begin{caution}[Correlation only sees straight lines]
$r \approx 0$ does not mean ``unrelated''. For $y = x^2$ on $x$ symmetric
about zero, $r = 0$ exactly while $y$ is a perfect function of $x$. The next
example is the same trap in a milder form.
\end{caution}
...
\]
For the length column, Equation~\eqref{eq:corr} by hand: $\bar\ell = 25$ and
$\overline{\text{price}} = 2225$, so the deviations are $(-15, -5, 5, 15)$ and
$(-225, -725, 775, 175)$, giving
\[
  S_{xy} = 13\,500, \quad S_{xx} = 500, \quad S_{yy} = 1\,207\,500, \qquad
  r = \frac{13\,500}{\sqrt{500 \times 1\,207\,500}} = 0.5494 .
\]
A straight line on length alone would therefore have
$R^2 = 0.5494^2 = 0.3019$.
...
leaves $\mathrm{SS}_{\mathrm{res}} = 415\,604$ of the
$\mathrm{SS}_{\mathrm{tot}} = 1\,207\,500$, so by Equation~\eqref{eq:r2}
\[
  R^2 = 1 - \frac{415\,604}{1\,207\,500} = 0.6558 ;
\]
add the column $\ell w$ and $\mathrm{SS}_{\mathrm{res}}$ falls to zero, so
$R^2 = 1.0000$.
\end{worked}
...
np.corrcoef(price, length)[0, 1]             # likewise width, length * width
...
\subsection{Worked: five points, three features}
\label{sec:pcahand}

\begin{worked}[PCA with no library at all]
Take the five rows
\[
  (1,2,5),\quad (2,3,4),\quad (3,5,6),\quad (4,4,6),\quad (5,6,4),
\]
so each observation now has three features rather than two. The mean is
$(3,4,5)$ and the centred rows are
\[
  (-2,-2,0),\quad (-1,-1,-1),\quad (0,1,1),\quad (1,0,1),\quad (2,2,-1).
\]
The six sums we need are
\[
  \textstyle\sum x_1^2 = 10,\quad \sum x_2^2 = 10,\quad \sum x_3^2 = 4,
  \qquad
  \sum x_1x_2 = 9,\quad \sum x_1x_3 = 0,\quad \sum x_2x_3 = 0 .
\]
Check the two zeros by hand, because everything below depends on them:
$\sum x_1x_3 = (-2)(0) + (-1)(-1) + (0)(1) + (1)(1) + (2)(-1)
= 0 + 1 + 0 + 1 - 2 = 0$. Dividing each sum by $n - 1 = 4$:
\begin{equation}
  \label{eq:cov}
  S = \begin{pmatrix}
...
\medskip
\textbf{Step 1: the characteristic equation.} The eigenvalues of $S$ are the
$\lambda$ for which $S - \lambda I$ is singular, that is
$\det(S - \lambda I) = 0$:
\[
  \det\!\begin{pmatrix}
    2.5-\lambda & 2.25 & 0\\
  ...
  \end{pmatrix} = 0 .
\]
Expand along the third row. Its only non-zero entry is $1-\lambda$ in position
$(3,3)$, whose cofactor is the top-left $2\times2$ minor, so the determinant
factorises immediately:
\begin{equation}
  \label{eq:chareq}
  (1-\lambda)\Bigl[(2.5-\lambda)^2 - (2.25)^2\Bigr] = 0 .
\end{equation}
\textbf{That factorisation is the block structure showing up in the algebra.}
A general $3\times3$ would leave an irreducible cubic; here the zeros in the
third row and column hand us one root for free.

\medskip
\textbf{Step 2: the three roots.} The first factor gives $\lambda = 1$
directly. For the second, expand the bracket:
\[
  (2.5-\lambda)^2 - 5.0625 = \lambda^2 - 5\lambda + 6.25 - 5.0625
                           = \lambda^2 - 5\lambda + 1.1875 ,
\]
and apply the quadratic formula:
\[
  \lambda = \frac{5 \pm \sqrt{25 - 4(1.1875)}}{2}
          = \frac{5 \pm \sqrt{20.25}}{2}
          = \frac{5 \pm 4.5}{2}
          \;\Longrightarrow\; \lambda = 4.75 \ \text{ or } \ 0.25 .
\]
The discriminant is a perfect square, $\sqrt{20.25} = 4.5$ exactly, which is
why this example works with a pen. Multiplying \eqref{eq:chareq} out gives the
cubic in full:
\begin{equation}
  \label{eq:cubic}
  \lambda^3 - 6\lambda^2 + 6.1875\lambda - 1.1875 = 0 .
\end{equation}

\medskip
\textbf{Step 3: check the roots against the invariants.} Do not skip this ---
it catches arithmetic slips before they propagate. For any $3\times3$ matrix
the coefficients of \eqref{eq:cubic} are the three invariants:
\[
  \begin{array}{lll}
    \sum\lambda_j = \operatorname{tr}(S) : & 4.75 + 1 + 0.25 = 6.0, &
      \operatorname{tr}(S) = 2.5+2.5+1 = 6.0 \ \checkmark\\[2pt]
    \sum_{i<j}\lambda_i\lambda_j : & 4.75(1) + 4.75(0.25) + 1(0.25) = 6.1875,
      & \text{coefficient } 6.1875 \ \checkmark\\[2pt]
    \prod\lambda_j = \det(S) : & 4.75 \times 1 \times 0.25 = 1.1875, &
      \det(S) = 1.1875 \ \checkmark
  \end{array}
\]

\medskip
\textbf{Step 4: an eigenvector for each root}, by solving
$(S - \lambda I)v = 0$.

For $\lambda_1 = 4.75$:
\[
  S - 4.75I = \begin{pmatrix}
    -2.25 & 2.25 & 0\\ 2.25 & -2.25 & 0\\ 0 & 0 & -3.75
  \end{pmatrix}.
\]
Row 3 reads $-3.75\,v_3 = 0$, so $v_3 = 0$. Row 1 reads
$-2.25\,v_1 + 2.25\,v_2 = 0$, so $v_1 = v_2$. Row 2 is row 1 times $-1$ and
adds nothing --- the matrix has rank 2, so the solution space is a single
line, as it must be for a simple root. Taking $v_1 = 1$ gives $v = (1,1,0)$,
and dividing by $\lVert v\rVert = \sqrt2$:
$u_1 = \frac{1}{\sqrt2}(1,1,0)$.

For $\lambda_2 = 1$:
\[
  S - I = \begin{pmatrix}
    1.5 & 2.25 & 0\\ 2.25 & 1.5 & 0\\ 0 & 0 & 0
  \end{pmatrix}.
\]
The top-left $2\times2$ has determinant $1.5^2 - 2.25^2 = -2.8125 \neq 0$, so
rows 1 and 2 force $v_1 = v_2 = 0$. Row 3 is entirely zero, leaving $v_3$
free. Hence $u_2 = (0,0,1)$, already of unit length.

For $\lambda_3 = 0.25$:
\[
  S - 0.25I = \begin{pmatrix}
    2.25 & 2.25 & 0\\ 2.25 & 2.25 & 0\\ 0 & 0 & 0.75
  \end{pmatrix}.
\]
Row 3 gives $v_3 = 0$; row 1 gives $v_1 + v_2 = 0$, so $v_2 = -v_1$. Taking
$v_1 = 1$: $u_3 = \frac{1}{\sqrt2}(1,-1,0)$.

\medskip
\textbf{Step 5: verify.} Multiply each one back through $S$:
\[
  S\begin{pmatrix}1\\1\\0\end{pmatrix}
    = \begin{pmatrix}2.5+2.25\\2.25+2.5\\0\end{pmatrix}
    = \begin{pmatrix}4.75\\4.75\\0\end{pmatrix}
    = 4.75\begin{pmatrix}1\\1\\0\end{pmatrix},
  \qquad
  S\begin{pmatrix}1\\-1\\0\end{pmatrix}
    = \begin{pmatrix}0.25\\-0.25\\0\end{pmatrix}
    = 0.25\begin{pmatrix}1\\-1\\0\end{pmatrix},
\]
and $S(0,0,1)^{\!\top} = (0,0,1)^{\!\top}$. The three are mutually
orthogonal --- $u_1 \cdot u_3 = \frac{1}{2}(1 - 1 + 0) = 0$, and $u_2$ has a
zero in both of the positions where $u_1$ and $u_3$ are non-zero --- which is
guaranteed, because a symmetric matrix always has an orthogonal eigenbasis.

Sorting by eigenvalue:
\[
  \begin{array}{lll}
    \lambda_1 = 4.75, & u_1 = \tfrac{1}{\sqrt2}(1,1,0), & 4.75/6 = 0.7917\\[2pt]
    \lambda_2 = 1.00, & u_2 = (0,0,1),                  & 1.00/6 = 0.1667\\[2pt]
    \lambda_3 = 0.25, & u_3 = \tfrac{1}{\sqrt2}(1,-1,0),& 0.25/6 = 0.0417
  \end{array}
\]

\medskip
\textbf{Step 6: the scores.} These are the centred rows dotted with each
$u_j$. On PC1, $u_1$ makes the score $(x_1 + x_2)/\sqrt2$:
$-2.8284,\ -1.4142,\ 0.7071,\ 0.7071,\ 2.8284$. Their sample variance is
$(8 + 2 + 0.5 + 0.5 + 8)/4 = 4.75 = \lambda_1$, as promised. On PC2 the score
is just the centred third feature, $(0,-1,1,1,-1)$, with variance
$4/4 = 1 = \lambda_2$.
\end{worked}

\begin{caution}[The shortcut, and why it is worth knowing you are using one]
A symmetric $2\times2$ with \emph{equal} diagonal entries always has
eigenvectors $\frac{1}{\sqrt2}(1,\pm1)$ and eigenvalues $a \pm b$, which reads
off $4.75$ and $0.25$ from $2.5 \pm 2.25$ in one line. That is a genuine
shortcut and you may use it --- but only once you can produce
Steps 1--4 without it, because the moment the diagonal entries differ, or the
matrix is not $2\times2$ after a block split, the shortcut is silent about the
answer and the characteristic equation is not.
\end{caution}

\begin{keyidea}[An uncorrelated feature is already a component]
$u_2 = (0,0,1)$ is not a mixture of anything --- it \emph{is} the third
feature. PCA only has work to do where columns are correlated; where they are
not, it leaves them alone. That is the honest answer to ``what does PCA
actually do'': it rotates within the correlated part of the data and ignores
the rest.
\end{keyidea}

\begin{figure}[tb]
  \centering
  \includegraphics[width=\textwidth]{pcahand.pdf}
  \caption{The five points in three dimensions; the vertical axis is $x_3$
  and the orange marker is the mean. Left: the three eigenvectors drawn at
  length $\sqrt{\lambda_j}$ --- PC2 stands vertically, because $x_3$ is
  uncorrelated with the other two features. Middle and right: reconstructions
  from one and from two components, with red lines showing the error on each
  point.}
  \label{fig:pcahand}
\end{figure}
...
for k in (1, 2):
    pk = PCA(n_components=k).fit(P)
    R = pk.inverse_transform(pk.transform(P))
\end{lstlisting}
\begin{codeout}
covariance matrix
 [[2.5  2.25 0.  ]
  [2.25 2.5  0.  ]
  [0.   0.   1.  ]]
eigenvalues [4.75 1.   0.25]   ratios [0.791667 0.166667 0.041667]
sklearn components_ (rows)
 [[ 0.707107  0.707107  0.      ]
  [ 0.        0.        1.      ]
  [ 0.707107 -0.707107 -0.      ]]
k=1 error 5.000000 = (n-1)*sum(lambda_{j>k}) = 5.000000  (4 distinct points of 5)
k=2 error 1.000000 = (n-1)*sum(lambda_{j>k}) = 1.000000  (4 distinct points of 5)
\end{codeout}

Three things to notice. The library's second component is exactly $(0,0,1)$,
the same answer the block structure gave us by hand. Both reconstructions
collapse five points onto four distinct ones, because $(3,5,6)$ and $(4,4,6)$
differ only along $u_3$: dropping a component \textbf{merges points that
differed only along it}, which is what ``lossy'' means. And both errors match
$(n-1)\sum_{j>k}\lambda_j$ exactly --- $4(1.00+0.25) = 5.00$ and
$4(0.25) = 1.00$ --- which is the general identity of the next section.
...
Lecture~12 treats regularisation properly; this section needs only enough of
it to use the lasso as a feature selector.

\paragraph{Coefficients and what a zero means.} A linear model predicts
$\hat y = w_0 + w_1 x_1 + \dots + w_d x_d$. Each $w_j$ is a
\textbf{coefficient}: how much the prediction moves when feature $j$ rises by
one unit. If $w_j = 0$ the feature is multiplied by zero and plays no part in
any prediction. A model with some coefficients at exactly zero has therefore
\emph{selected} the others, and counting the non-zero coefficients counts the
features kept.

\paragraph{Penalties.} Plain least squares minimises only the prediction
error. A \textbf{penalised} model adds a charge for large coefficients, with
a strength $\alpha \ge 0$ that you choose:
\begin{align}
  \text{ridge (L2 penalty):}\quad
    & \min_w \; \sum_i (y_i - \hat y_i)^2 + \alpha \sum_j w_j^2 ,
    \label{eq:ridge}\\
  \text{lasso (L1 penalty):}\quad
    & \min_w \; \sum_i (y_i - \hat y_i)^2 + \alpha \sum_j |w_j| .
    \label{eq:lasso}
\end{align}
The names come from the two ways of measuring the size of $w$: the L2 norm
uses squares, the L1 norm absolute values. At $\alpha = 0$ both are ordinary
least squares; as $\alpha$ grows, the coefficients are pulled towards zero.
Refitting the same model at a range of $\alpha$ values traces out a
\textbf{path}, which is what the output below does.

\paragraph{Why only L1 produces exact zeros.} Near zero, the square $w_j^2$
becomes tiny, so the ridge penalty stops pushing a coefficient that is
already small: ridge makes every coefficient smaller but almost never exactly
zero. The absolute value $|w_j|$ charges the same $\alpha$ per unit of
$w_j$ all the way down to zero, so a feature whose contribution to the fit is
worth less than that charge is set to exactly $0$. That is why the lasso
\emph{selects} and ridge only \emph{shrinks}. Because the selection happens
as part of fitting, rather than in a separate step before (a filter) or around
(a wrapper) the model, it is called \textbf{embedded} selection.

\paragraph{Why standardise first.} The penalty adds up $|w_j|$ across
features. A feature measured in large units needs only a small coefficient,
so it is penalised less for the same real effect. Standardising puts every
feature on the same scale, which makes the comparison fair
(Section~\ref{sec:scale} made the same point for distances and
gradients).

\paragraph{The data.} \texttt{load\_diabetes} has $442$ patients and ten
baseline measurements: \texttt{age}, \texttt{sex}, \texttt{bmi} (body mass
index), \texttt{bp} (average blood pressure) and six blood-serum readings ---
\texttt{s1} total cholesterol, \texttt{s2} LDL, \texttt{s3} HDL, \texttt{s4}
total cholesterol over HDL, \texttt{s5} the log of serum triglycerides and
\texttt{s6} blood sugar. The target is a quantitative measure of disease
progression one year later. In the output, \emph{nonzero} is the number of
coefficients the lasso kept (named when there are six or fewer) and
\emph{CV R2} is the mean $R^2$ (Equation~\eqref{eq:r2}) over five-fold
cross-validation, with the scaler and the lasso refitted inside each fold.

\begin{caution}[scikit-learn scales the lasso's error term]
\texttt{Lasso} actually minimises
$\frac{1}{2n}\sum_i (y_i - \hat y_i)^2 + \alpha \sum_j |w_j|$, while
\texttt{Ridge} uses Equation~\eqref{eq:ridge} as written. The same number
$\alpha$ therefore means a much stronger penalty in \texttt{Lasso} than in
\texttt{Ridge}; compare each model with itself across $\alpha$, not the two
at one value.
\end{caution}
...
Ridge ($\alpha = 1$) keeps all ten and scores the same: \textbf{L1
selects, L2 only shrinks.} Because the selection depends on $\alpha$, the selection
...
All links were checked on 22 August 2026; the scikit-learn metrics and NumPy
\texttt{corrcoef} pages were added and checked on 16 September 2026. Free resources are marked
\textbf{[free]}.
...
\begin{itemize}[leftmargin=*]
  ...
  \item James, Witten, Hastie, Tibshirani and Taylor, \emph{An Introduction to
    Statistical Learning with Python}. \textbf{[free]}
    \S3.1.3 assessing the accuracy of a fitted line --- the $R^2$ statistic
    and why it equals the squared correlation for a single input (the two
    measures used from Section~\ref{sec:r2corr} on); \S6.3 dimension
...
\begin{itemize}[leftmargin=*]
  ...
  \item scikit-learn user guide, \emph{Metrics and scoring} (\S3.4) --- the
    $R^2$ score section gives Equation~\eqref{eq:r2} and its edge cases; and
    the NumPy reference for \texttt{numpy.corrcoef}, which computes
    Equation~\eqref{eq:corr}. \textbf{[free]}
